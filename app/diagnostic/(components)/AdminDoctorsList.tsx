...
const AdminDoctorLists = ({
  ...
  const removeDiagnostic = async (id: string | undefined) => {
    ...
    if (res.isConfirmed) {
      try {
        ...
        if (index === 0) {
          const newarr = adminDoctors.filter((dg) => dg.id !== id);
          setAdminDoctors(newarr);
        } else {
          const newarr = outSideDoctors.filter((dg) => dg.id !== id);
          setOutSideDoctors(newarr);
        }
      } catch (error: any) {
        Swal.fire({
  ...
  return (
    <div className={`grid  items-center my-3 text-lg grid-cols-5`}>
      ...
      <div>{designation || "not set"}</div>
      <div>{specialist || "not set"}</div>
      <div>{division}</div>
      <button
        onClick={() => {
          removeDiagnostic(id);
...
export default AdminDoctorLists;
